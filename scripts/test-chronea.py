...
def main():
    ...
    for filename in ['test-chronea-v3.py','test-chronea-ui.py','check-chronea-final.py']:
    ...
    report={'version':'3.0.0','unit':unit,'browser':browser,'acceptance':final,
            'total':len(unit)+len(browser)+len(final['checks']),
            'failed':sum(not row['ok'] for row in unit+browser+final['checks']),
