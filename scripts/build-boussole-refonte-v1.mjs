...
const CLASSIC_PATH = path.join(APP_DIR, "boussole-pro.html");
const REFONTE_PATH = path.join(APP_DIR, "boussole-pro-refonte.html");
...
const refonteTemplate = await readFile(REFONTE_PATH, "utf8");
...
await writeFile(REFONTE_PATH, builtHtml);
...
await writeFile(path.join(TMP_DIR, "prototype-data-summary.json"), `${JSON.stringify({
  ...manifest,
  output: {
    htmlPath: path.relative(ROOT, REFONTE_PATH),
...
console.log(JSON.stringify({
  status: "built",
  html: path.relative(ROOT, REFONTE_PATH),
